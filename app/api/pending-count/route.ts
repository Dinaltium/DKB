...
export async function GET() {
  try {
    ...
    try {
      ...
      console.warn("[pending-count] DB query failed, returning 0:", dbErr instanceof Error ? dbErr.message : dbErr);
      return NextResponse.json({ count: 0 });
    }
  } catch (authErr) {
    // Auth failure (JWT error, session decode error) — return 0 quietly
    console.warn("[pending-count] Auth check failed:", authErr instanceof Error ? authErr.message : authErr);
    return NextResponse.json({ count: 0 });
  }
}