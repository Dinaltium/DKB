...
const RULES: Record<string, DocRules> = {
	aadhaar: {
		...
		name: [/(?:name|नाम)[:\s]+([A-Z][a-zA-Z\s]{2,35})/i],
		dob: [
	...
	driving_license: {
		...
		name: [/(?:name|holder|licensee)[:\s]+([A-Z][a-zA-Z\s]{2,35})/i],
		valid_till: [
			/(?:valid\s*(?:till|upto|through)|expiry|exp\.?)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})/i,
		],
		dob: [/(?:dob|date\s*of\s*birth)[:\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})/i],
	},
	...
	insurance: {
		...
		vehicle_number: [/\b([A-Z]{2}[-\s]\d{2}[-\s][A-Z]{1,3}[-\s]\d{4})\b/i],
		valid_till: [
	...
	bus_ticket: {
		...
		seat_number: [/(?:seat|seat\s*no)[:\s#]*([A-Z]?\d{1,3}[A-Z]?)\b/i],
	},
};
