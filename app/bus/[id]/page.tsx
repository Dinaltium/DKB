...
import { getBusById, getRouteStopIds, getStopsForBus } from "@/lib/db/queries";
...
export default async function BusDetailPage({ params, searchParams }: Props) {
	...
	const currentStopObj =
		stops.find((s) => s.name === currentStopName || s.id === currentStopName) ??
		stops[0];
		...
		>
			...
			>
				<BusDetailClient
					...
					session={
						session ? { id: session.user.id, role: session.user.role } : null
					}
				/>
			</Suspense>
		</AppShell>
