...
export function PaymentDrawer({ busNumber, amount, onSuccess }: PaymentDrawerProps) {
  ...
  const processPayment = () => {
    ...
    setTimeout(() => {
      ...
      const result: Receipt = { status: success ? "success" : "failed", transactionId };
      setReceipt(result);
      if (success) {
        toast.success("Payment completed successfully");
        try {
          const payments = JSON.parse(localStorage.getItem("buslink_payments") || "[]") as unknown[];
          payments.push({ busNumber, amount, transactionId, timestamp: Date.now() });
  ...
  const reset = () => { setReceipt(null); setUpiId(""); setPayerName(""); };

  const inputStyle = { background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--input-text)" };

  if (!open) {
    return (
      <button onClick={() => setOpen(true)} data-testid="open-payment-drawer-button"
  ...
  return (
    <>
      <div className="fixed inset-0 z-40 bg-black/40" onClick={() => { setOpen(false); reset(); }} />
      <div
      ...
      >
        <div className="mb-1 flex items-center justify-between">
          ...
          <button onClick={() => { setOpen(false); reset(); }} style={{ color: "var(--text-muted)" }} className="rounded-full p-1 hover:opacity-80">
            ✕
          </button>
        </div>
        <p className="mb-4 text-sm" style={{ color: "var(--text-muted)" }} data-testid="payment-drawer-description">
          Bus {busNumber} &bull; Pay ₹{amount}
        </p>

        <div className="space-y-3">
          <input data-testid="payment-payer-name-input" type="text" placeholder="Your name" value={payerName}
            onChange={(e) => setPayerName(e.target.value)}
            className="h-11 w-full border-2 px-3 text-sm outline-none" style={inputStyle} />
          <input data-testid="payment-upi-id-input" type="text" placeholder="UPI ID (example@upi)" value={upiId}
          ...
          {receipt && (
            <div
              ...
              style={{ borderColor: receipt.status === "success" ? "#10b981" : "#ef4444" }}
            >
              ...
              <p data-testid="payment-result-transaction-id" className="mt-2 font-mono text-xs" style={{ color: "var(--text-muted)" }}>
                Transaction: {receipt.transactionId}
              </p>
            </div>
          )}

          <button onClick={processPayment} disabled={loading} data-testid="payment-submit-button"
