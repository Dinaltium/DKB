...
export function SearchClient({ buses, stops, operators }: Props) {
	...
	const results = useMemo<SearchResult[]>(() => {
		...
		return buses.flatMap((bus) => {
			...
			if (maxFare && fare > Number.parseInt(maxFare, 10)) return [];
			if (minSeats && availableSeats < Number.parseInt(minSeats, 10)) return [];
			if (timeFilter) {
		...
		<>
			...
			>
				<div className="grid gap-3 md:grid-cols-2">
					...
					<div className="flex flex-col gap-1">
						...
						<input
							...
							className="theme-input brutal-transition brutal-focus h-12 w-full rounded-none border-2 border-foreground px-3 text-sm outline-none neo-shadow"
							style={inputStyle}
						/>
					</div>
					...
					<div className="flex flex-col gap-1">
						...
						<input
							...
							className="theme-input brutal-transition brutal-focus h-12 rounded-none border-2 border-foreground px-3 text-sm outline-none neo-shadow"
							style={inputStyle}
						/>
					</div>
				...
				<button
					...
					className="theme-btn-primary brutal-transition brutal-hover mt-4 inline-flex h-11 items-center gap-2 rounded-none border-2 px-6 text-sm font-bold uppercase tracking-wider neo-shadow hover:opacity-80"
				>
			...
			>
				...
				{results.map((item) => (
					<div
						...
						className="theme-panel rounded-none border-2 neo-shadow"
						style={{
							borderColor: "var(--border-default)",
						}}
					>
						<div className="grid gap-4 p-5 md:grid-cols-[1.4fr_1fr_auto] md:items-center">
							...
							>
								...
								<div className="flex items-center justify-between gap-2">
									...
									<strong className="theme-text-teal">
										&#8377;{item.fare}
									</strong>
								</div>
								<div className="flex items-center justify-between gap-2">
							...
							<Link
								...
								className="theme-btn-dark brutal-transition brutal-hover inline-flex h-10 items-center justify-center rounded-none border-2 px-5 text-xs font-bold uppercase tracking-wider neo-shadow hover:opacity-80"
							>
								View Bus
							</Link>
						</div>
