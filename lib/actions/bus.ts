...
export async function setOperatorApprovalAction(
	operatorId: string,
	approved: boolean,
) {
	const session = await auth();
...
export async function submitBusRequestAction(data: {
	...
	const op = await getOperatorByUserId(session.user.id);
	if (!op?.approved) return { success: false, error: "Operator not approved" };
...
export async function rejectBusRequestAction(
	requestId: string,
	adminNote?: string,
) {
	const session = await auth();
...
export async function importStopsAction(
	...
	for (const s of stopsData) {
		if (
			...
			return {
				success: false,
				error: `Invalid stop data: ${JSON.stringify(s)}`,
			};
		}
	}
