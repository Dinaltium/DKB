...
function findDocRect(edges: Uint8Array, w: number, h: number): Rect | null {
	...
	const px = Math.round(rw * PADDING_RATIO),
		py = Math.round(rh * PADDING_RATIO);
	return {
		x: Math.max(0, lft - px),
		y: Math.max(0, top - py),
		...
		try {
			...
			await worker.setParameters({
				tessedit_pageseg_mode: String(
					cfg.psm,
				) as unknown as import("tesseract.js").PSM,
