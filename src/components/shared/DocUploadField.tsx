...
function findDocRect(edges: Uint8Array, w: number, h: number): Rect | null {
	...
	const px = Math.round(rw * PADDING_RATIO);
	const py = Math.round(rh * PADDING_RATIO);
	return {
		x: Math.max(0, lft - px),
		y: Math.max(0, top - py),
...
async function detectAndCrop(file: File): Promise<DetectionResult> {
	...
	if (!rect) {
		...
		const ow = Math.round(img.width * os);
		const oh = Math.round(img.height * os);
	...
	const ow = Math.round(rawW * os);
	const oh = Math.round(rawH * os);
...
function drawOverlay(
	...
	// Green border
	ctx.strokeStyle = "var(--status-running-text)";
	ctx.lineWidth = 2;
	ctx.beginPath();
	ctx.moveTo(pts[0].x, pts[0].y);
	pts.slice(1).forEach((p) => ctx.lineTo(p.x, p.y));
	...
	// Corner handles
	ctx.fillStyle = "var(--status-running-text)";
...
const DOC_CONFIG: Record<
	OcrDocType,
	{ psm: number; whitelist: string | null }
> = {
	aadhaar: {
		psm: 6,
		whitelist:
...
export function DocUploadField({
	...
	const handleFile = async (file: File) => {
		...
		try {
			...
			await new Promise<void>((res) => {
				tmp.onload = () => {
					natSizeRef.current = { w: tmp.naturalWidth, h: tmp.naturalHeight };
					res();
				};
				tmp.src = objUrl;
			});
			...
			await worker.setParameters({
				tessedit_pageseg_mode: String(cfg.psm) as any,
	...
	const isScanning =
		scanState.status === "detecting" || scanState.status === "scanning";
	const progressPct =
		scanState.status === "detecting"
			? 8
		...
		<div className="space-y-2">
			...
			<div className="flex gap-2">
				...
				<button
					...
					className="flex h-11 w-11 shrink-0 items-center justify-center rounded-none border-2 border-foreground neo-shadow transition-all hover:translate-x-[4px] hover:translate-y-[4px] hover:neo-shadow-none disabled:cursor-not-allowed disabled:opacity-50"
			...
			{preview && (
				<div
					className="rounded-none border-2 border-foreground p-2.5"
				...
				>
					<div className="flex items-start gap-3">
						...
						<div className="flex min-w-0 flex-1 flex-col gap-1.5">
							...
							{isScanning && (
								<div className="space-y-1.5">
									...
									<div
										className="h-1 w-full overflow-hidden rounded-none border border-foreground"
							...
							{scanState.status === "done" && (
								<div className="space-y-0.5">
									...
									<span
										...
										style={{ color: confColor(scanState.confidence) }}
									>
										Confidence: {scanState.confidence}%
										{scanState.confidence < 60
											? " — try a clearer photo"
											: scanState.confidence < 75
												? " — double-check value"
												: ""}
									</span>
									<button
						...
						<button
							type="button"
							onClick={handleClear}
							className="flex h-6 w-6 shrink-0 items-center justify-center rounded-none border-2 border-foreground transition-colors hover:bg-foreground hover:text-background"
							title="Clear"
						>
							<X className="h-3 w-3" />
						</button>
