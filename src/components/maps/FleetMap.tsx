...
const STATUS_RING: Record<string, string> = {
	Running: "var(--status-running-border)",
	Delayed: "var(--status-delayed-border)",
	"Not Running": "var(--status-stopped-border)",
};
...
export default function FleetMap() {
	...
	const addMarker = (
		...
		const ring = STATUS_RING[status] ?? "var(--status-running-border)";
        ...
        <div style="position:absolute;inset:4px;border-radius:50%;background:${fill};border:2.5px solid var(--color-navy);box-shadow:0 2px 8px rgba(0,0,0,0.35)"></div>
        <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:'Barlow Condensed',sans-serif;font-size:9px;font-weight:800;color:var(--color-navy);pointer-events:none;line-height:1">${label}</div>
        <div style="position:absolute;top:1px;right:1px;width:8px;height:8px;border-radius:50%;background:${ring};border:1.5px solid var(--text-inverse)"></div>
      </div>`,
		});

		const tooltipHtml = `<div style="font-family:'Barlow Condensed',sans-serif;background:var(--color-navy);color:var(--text-inverse);border-radius:0;padding:6px 10px;font-size:13px;font-weight:700;white-space:nowrap;box-shadow:4px 4px 0 var(--color-amber);pointer-events:none">
