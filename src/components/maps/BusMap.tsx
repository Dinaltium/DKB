...
export default function BusMap({ stops, livePosition }: BusMapProps) {
	...
	useEffect(() => {
        ...
        <div style="position:relative;width:28px;height:28px">
          <div style="position:absolute;inset:0;border-radius:50%;background:color-mix(in srgb, var(--color-amber) 28%, transparent);animation:ping 1.5s ease-in-out infinite;"></div>
          <div style="position:absolute;inset:6px;border-radius:50%;background:var(--color-amber);border:3px solid var(--color-navy);box-shadow:0 2px 6px rgba(0,0,0,0.3)"></div>
        </div>`,
			className: "",
			iconSize: [28, 28],
			iconAnchor: [14, 14],
		...
		<div className="ticket-stub overflow-hidden rounded-none border-2 border-foreground neo-shadow">
			...
			>
				<div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide">
					<span className="theme-bg-amber inline-block h-2 w-2 animate-pulse rounded-full" />
				...
				{!isFollowing && livePosition && (
					<button
						...
						className="theme-btn-secondary brutal-transition brutal-hover flex items-center gap-1.5 rounded-none border-2 px-3 py-1 text-xs font-bold uppercase tracking-wide neo-shadow"
					>
				...
				{isFollowing && livePosition && (
					...
					>
						<span
							...
							style={{ background: "var(--status-running-border)" }}
						/>
						Following bus
					</span>
				)}
