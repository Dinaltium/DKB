...
export interface DetectionResult {
  ...
  const MIN_DOC_RATIO  = 0.10;   // doc must be ≥ 10% of image area
  ...
  export async function detectAndCropDocument(
    ...
    const detectScale = Math.min(1, MAX_PROCESS_PX / Math.max(img.width, img.height));
  ...
  function findDocumentRect(
    ...
    for (let y = 0; y < cy; y++)     { if (rowDensity[y] > rowThresh) { top    = Math.max(0, y - STRIP); break; } }
    for (let y = h-1; y > cy; y--)   { if (rowDensity[y] > rowThresh) { bottom = Math.min(h-1, y + STRIP); break; } }
    for (let x = 0; x < cx; x++)     { if (colDensity[x] > colThresh) { left   = Math.max(0, x - STRIP); break; } }
    for (let x = w-1; x > cx; x--)   { if (colDensity[x] > colThresh) { right  = Math.min(w-1, x + STRIP); break; } }
